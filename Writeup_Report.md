...
* In a nutshell, trajectory generation is performed by combining waypoints from previous path trajectory (waypoints that have not been covered by the ego car) and a new set of way points generated by using a spline curve
* The inputs to generating a spline curve are as follows:
  - Last two waypoints points from the previous path trajectory
  - To the above two waypoints 3 additonal waypoints are added at increments of 30, 60, 90 meters
  - Addition of end points from previous path trajectory to the input of spline function will result in smooth transition between successive path trajectories
* New path trajectory is gnerated 
